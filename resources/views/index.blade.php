    ...
    <section x-data="{
        ...
        @endphp
            <div x-show="activeSlide === {{ $index }}" x-transition:enter="transition ease-out duration-1000"
                ...
                <a href="{{ route('portal.show', $item->slug) }}" class="relative block w-full h-full">
                    ...
                    <div class="relative h-full flex items-center justify-center">
                        <div class="px-4 text-center max-w-5xl mt-32">
                            <span
                                class="inline-block {{ $kategoriColor }} py-3 px-4 rounded-lg text-2xl font-semibold tracking-widest mb-3 uppercase text-white">
                                {{ $item->Kategori }}
                            </span>
                            ...
                            <p class="text-md mt-6 md:text-2xl text-gray-300">
                                Oleh {{ $item->Penulis }} • {{ $item->created_at->format('d F Y') }}
                            </p>
                        </div>
                    </div>
                </a>
            ...
            <div class="absolute right-3 md:right-10 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-3">
                @foreach ($rekomendasi as $index => $item)
                   <button
  @click="activeSlide = {{ $index }}" :class="activeSlide === {{ $index }} ? 'bg-blue-400 h-10 w-2' : 'bg-white/10 h-2 w-2'" class="rounded-full transition-all duration-500 shadow-sm">
 @endforeach
            </div>
        @endforeach
                ...
                <a href="{{ route('portal.show', $item->slug) }}" class="block">
                    <div class="w-80 flex-shrink-0 bg-transparent flex flex-col gap-4 snap-center">
                        <img src="{{ url('storage/' . $item->Gambar) }}"
                            class="w-full h-auto object-cover rounded-lg aspect-[16/9]" alt="{{ $item->Judul }}">
                        <p class="{{ $kategoriColor }} font-bold">{{ $item->Kategori }}</p>
                        ...
                        <div class="flex gap-3 justify-start text-gray-500 text-sm">
                         <p>{{ $item->Penulis }} | {{ $item->created_at->format('d/m/y') }} <span
                                    class="mx-1">|</span> <i class="far fa-eye"></i> {{ $item->view ?? 0 }}</p>
                        </div>
                    </div>
                </a>
            @endforeach
    ...
    <section class="px-12 md:px-32 py-12 bg-secondary text-white">
        ...
        <div class="flex flex-wrap gap-5">
            @foreach ($portals->where('Kategori', 'ALAM')->take(3) as $item)
                @php $thisColor = $colorMap[$item->Kategori] ?? 'text-gray-500'; @endphp
                <a href="{{ route('portal.show', $item->slug) }}" class="block">
                    <div class="w-80 flex-shrink-0 bg-transparent flex flex-col gap-4">
                        ...
                        <p class="{{ $thisColor }} font-bold">{{ $item->Kategori }}</p>
                        <h2 class="font-bold text-xl line-clamp-2">{{ $item->Judul }}</h2>
                        <div class="flex gap-3 justify-start text-gray-500 text-sm">
                            <p>{{ $item->Penulis }} | {{ $item->created_at->format('d/m/y') }} <span
        ...
        <div class="flex flex-wrap gap-5">
            @foreach ($portals->where('Kategori', 'BERDAYA')->take(3) as $item)
                @php $thisColor = $colorMap[$item->Kategori] ?? 'text-gray-500'; @endphp
                <a href="{{ route('portal.show', $item->slug) }}" class="block">
                    <div class="w-80 flex-shrink-0 bg-transparent flex flex-col gap-4">
                        <img src="{{ url('storage/' . $item->Gambar) }}" alt="{{ $item->Judul }}"
        ...
        <div class="flex justify-between items-center mb-8 mt-16">
            ...
            <a href="{{ route('portal.kategori', 'budaya') }}">
                <p class="font-semibold text-lg uppercase">Lihat Semua</p>
            </a>
        </div>
        <hr class="border-t border-white opacity-20 my-12">
        ...
        <div class="flex flex-wrap gap-5">
            @foreach ($portals->where('Kategori', 'BUDAYA')->take(3) as $item)
                @php $thisColor = $colorMap[$item->Kategori] ?? 'text-gray-500'; @endphp
                <a href="{{ route('portal.show', $item->slug) }}" class="block">
                    <div class="w-80 flex-shrink-0 bg-transparent flex flex-col gap-4">
                        <img src="{{ url('storage/' . $item->Gambar) }}" alt="{{ $item->Judul }}"
        ...
        <div class="flex justify-between items-center mb-8 mt-16">
            ...
            <a href="{{ route('portal.kategori', 'suara') }}">
                <p class="font-semibold text-lg uppercase">Lihat Semua</p>
            </a>
        </div>
        <hr class="border-t border-white opacity-20 my-12">
        ...
        <div class="flex flex-wrap gap-5">
            @foreach ($portals->where('Kategori', 'SUARA')->take(3) as $item)
                @php $thisColor = $colorMap[$item->Kategori] ?? 'text-gray-500'; @endphp
                <a href="{{ route('portal.show', $item->slug) }}" class="block">
                    <div class="w-80 flex-shrink-0 bg-transparent flex flex-col gap-4">
                        <img src="{{ url('storage/' . $item->Gambar) }}" alt="{{ $item->Judul }}"
